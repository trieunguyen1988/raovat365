...
function setPaymentMethod(duration) {
    var method = $('input[name="payment_method"]:checked').val();
    var turnOn = true;
    if(method == "1") {
        toggleCardInfo(!turnOn, duration);
    }
    else if(method == "2") {
        toggleCardInfo(turnOn, duration);
    }
...
function setNextPaymentDate() {
    var payment = $('input[name="basic_plan_id"]:checked').val();
    $('#next_payment_label').html($('#hidden_next_pay_date_'+payment).val());
    $('#next_payment_date').val($('#hidden_next_pay_date_'+payment).val());
}
function setPaymentAmount() {
    var payment = $('input[name="basic_plan_id"]:checked').val();
    var amount = $('#hidden_payment_amount_'+payment).val() * $('#num_of_shop').val();
    if(isNaN(amount)) {
        amount = 0;
    }
    $('#payment_amount_lable').html('¥' + numberWithCommas(amount));
    $('#payment_amount').val(amount);
}
function toggleCardInfo(isEnabled, duration) {
    if(isEnabled) {
        $('.transfer-group').show({duration:duration});
    }
    else {
        $('.transfer-group').hide({duration:duration});
    }
}